...
public class StatementControllerIntegrationTest {
	...
	@Autowired
	OpenLRSAuthenticationFilter filter;
	@Autowired
	XAPIHeaderFilter xapiHeaderFilter;
	@Autowired
	XAPIRequestValidationFilter xapiRequestValidationFilter;
	...
	public void setup() {
		...
		this.mockMvc = MockMvcBuilders.webAppContextSetup(this.wac)
				...
				.addFilter(xapiRequestValidationFilter, "/xAPI/*").build();

	}

	@Test
	public void thatStatementGetWithNoAuthorizationHeaderReturns401()
			throws Exception {
		this.mockMvc
				.perform(
						get("/xAPI/statements").accept(
								MediaType.APPLICATION_JSON)).andDo(print())
				.andExpect(status().isUnauthorized());
	}

	@Test
	public void thatStatementGetWithNoVersionHeaderReturns400()
			throws Exception {
		String basic = "test:test";
		final byte[] encodedBytes = Base64.encodeBase64(basic.getBytes());
		this.mockMvc
				.perform(
						get("/xAPI/statements").header("Authorization",
								"Basic " + new String(encodedBytes)).accept(
								MediaType.APPLICATION_JSON)).andDo(print())
				.andExpect(status().isBadRequest());
	}

	@Test
	public void thatStatementGetWithOAuthButNotConsumerKeyReturns401()
			throws Exception {
		this.mockMvc
				.perform(
						get("/xAPI/statements")
								.header("Authorization", "OAuth")
								.header(XApiConstants.XAPI_VERSION_HEADER,
										"someversion")
	...
	public void thatStatementReturnsHeaderWithVersion() throws Exception {
		...
		this.mockMvc
				...
				.andExpect(
						header().string("X-Experience-API-Version",
								"someversion"));
	}
	...
	@Test
	public void shouldHandleStatementWithPrettyMuchEverything()
			throws Exception {
